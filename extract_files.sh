...
# Find all .cs and .json files, excluding bin/ and obj/ directories and .sln files
find . -type f \( -name "*.cs" -o -name "*.json" \) ! -path "*/bin/*" ! -path "*/obj/*" ! -name "*.sln" | while read -r FILE; do
